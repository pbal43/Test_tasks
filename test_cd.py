# Файл должен лежать в основной рабочей директории (main)
...
while True:
    ld_1 = str.upper(str.strip(input('Введите наименование локального диска (на латинице), где создана тестовая директория,\nявляющейся основной, где также будут результаты:\n')))
    ld_2 = str.upper(str.strip(input('Введите наименование другого диска (на латинице), где создана побочная тестовая директория,\nнеобходимая для тестирования перехода между локальными дисками:\n')))
    if ld_1 == ld_2:
...
r = open(results, 'w')
r.write('Результаты тестирования команды CD (пункты соответствуют пунктам чек-листа):\n')
r.close()
...
while True:
    ...
    if m_1_p == 0:
        ...
        break
# 4. Переход из main на 1 уровень вверх (в родительский каталог) с помощью cd.. (слитно)
while True:
    fr_main_to_up = 'cd..'
...
while True:
    il_nome = 'cd'
    nome = os.system(il_nome)
    if nome == 0:
        calling_output_n = subprocess.check_output(il_nome, shell=True)
        nn = calling_output_n.decode('cp866')
        nnn = nn.rstrip()
        b_b = r'\test\main'
        bb = f'{ld_1}:' + b_b
        if nnn == bb:
            with open(results, 'a') as nome_rep:
                nome_rep.write('10. Успех. Отображение полного пути текущей директории успешно\n')
            break
    else:
        with open(results, 'a') as nome_rep:
            nome_rep.write('10. Баг! Отображение полного пути текущей директории некорректно / не осуществлено!\n')
        break
# 11. Использование нижнего регистра при указании локального диска для перехода (ранее - в верхнем)
while True:
    ...
    if m_r_plow == 0:
        with open(results, 'a') as m_r_plow_rep:
            m_r_plow_rep.write('11. Успех. Переход с указанием имени локального диска в нижнем регистре успешен\n')
        break
    else:
        with open(results, 'a') as m_r_plow_rep:
            m_r_plow_rep.write('11. Баг! Переход с указанием имени локального диска в нижнем регистре не осуществлен!\n')
        break

# 12. Попытка перехода на другой диск с обратным слешем указателя смены диска \d
        ...
        if zzz == tt:
            if j_j == error_1:
                with open(results, 'a') as m_od_w_rep:
                    m_od_w_rep.write('12. Успех. Переход на другой диск с помощью \d не осуществлен. Текст ошибки верен\n')
                break
    else:
        with open(results, 'a') as m_od_w_rep:
            m_od_w_rep.write('12. Баг! Переход на другой диск с помощью \d отработан некорректно или текст ошибки не верен!\n')
        break
# 13. Попытка перехода с указанием /d, но в директорию на этом же (стартовом) локальном диске
while True:
    ...
    if m_w_d == 0:
        with open(results, 'a') as m_w_d_rep:
            m_w_d_rep.write(r'13. Успех. Переход с указанием /d, но в директорию на текущем локальном диске успешен' + '\n')
        break
    else:
        with open(results, 'a') as m_w_d_rep:
            m_w_d_rep.write(r'13. Баг! Переход с указанием /d, но в директорию на текущем локальном диске не осуществлен!' + '\n')
        break
# 14. Попытка перехода на другой диск без /d
while True:
    ...
    if m_w_wd == 0:
        with open(results, 'a') as m_w_wd_rep:
            m_w_wd_rep.write(r'14. Успех. Попытка перехода на другой диск без /d не осуществлена'+ '\n')
        break
    else:
        with open(results, 'a') as m_w_wd_rep:
            m_w_wd_rep.write(r'14. Баг! Переход на другой диск без /d отработан некорректно!'+ '\n')
        break
# 15. Попытка перехода в несуществующие директории
        ...
        if sss == yy:
            if o_o == error_2:
                with open(results, 'a') as m_2_rep:
                    m_2_rep.write('15. Успех. Ошибка перехода в несуществующую директорию отработа верно. Текст ошибки релевантен\n')
                break
    else:
        with open(results, 'a') as m_2_rep:
            m_2_rep.write('15. Баг! Ошибка перехода в несуществующую директорию отработа неверно. Текст ошибки нерелевантен!\n')
        break
# 16. Попытка перехода в папку с пробелами в названии с указанием полного пути без кавычек:
while True:
    ...
    if m_lf == 0:
        with open(results, 'a') as m_lf_rep:
            m_lf_rep.write('16. Успех. Переход в папку с пробелом в названии с указанием полного пути без кавычек успешен\n')
        break
    else:
        with open(results, 'a') as m_lf_rep:
            m_lf_rep.write('16. Баг! Переход в папку с пробелом в названии с указанием полного пути без кавычек не осуществлен!\n')
        break
# 17. Попытка перехода в папку с пробелами в названии с указанием полного пути и с использованием кавычек:
while True:
    ...
    if m_lf_w == 0:
        with open(results, 'a') as m_lf_w_rep:
            m_lf_w_rep.write('17. Успех. Переход в папку с пробелом в названии с указанием полного пути и кавычек успешен\n')
        break
    else:
        with open(results, 'a') as m_lf_w_rep:
            m_lf_w_rep.write('17. Баг! Переход в папку с пробелом в названии с указанием полного пути и кавычек не осуществлен!\n')
        break
# 18. Переход на другой диск с использованием прямых слешей
while True:
    ...
    if m_od_rev == 0:
        with open(results, 'a') as m_od_rev_rep:
            m_od_rev_rep.write('18. Успех. Переход на другой диск с использованием прямых слешей в пути успешен\n')
        break
    else:
        with open(results, 'a') as m_od_rev_rep:
            m_od_rev_rep.write('18. Баг! Переход на другой диск с использованием прямых слешей в пути не осуществлен!\n')
        break
# 19. Переход из main во вложенный каталог - result с помощью указания полного пути на 1 уровень с использованием двойных слешей
while True:
    ...
    if m_r_p2 == 0:
        with open(results, 'a') as m_r_p2_rep:
            m_r_p2_rep.write('19. Успех. Переход в каталог, располеженный на текущем локальном диске, с указанием полного пути и двойных слешей успешен\n')
        break
    else:
        with open(results, 'a') as m_r_p2_rep:
            m_r_p2_rep.write('19. Баг! Переход в каталог, располеженный на текущем локальном диске, с указанием полного пути и двойных слешей не осуществлен!\n')
        break
# 20. Переход из main во вложенный каталог - result с помощью указания полного пути на 1 уровень с использованием двойных и одинарных слешей
while True:
    ...
    if m_r_pv == 0:
        with open(results, 'a') as m_r_pv_rep:
            m_r_pv_rep.write('20. Успех. Переход во вложенный каталог на 1 уровень успешен с указанием полного пути и двойных, и одинарных слешей\n')
        break
    else:
        with open(results, 'a') as m_r_pv_rep:
            m_r_pv_rep.write('20. Баг! Переход во вложенный каталог на 1 уровень не осуществлен с указанием полного пути и двойных, и одинарных слешей!\n')
        break
# 21. Попытка перехода в скрытую папку c указанием полного пути
while True:
    ...
    if m_r_ph == 0:
        with open(results, 'a') as m_r_ph_rep:
            m_r_ph_rep.write('21. Успех. Переход в скрытую папку c указанием полного пути успешен\n')
        break
    else:
        with open(results, 'a') as m_r_ph_rep:
            m_r_ph_rep.write('21.Баг! Переход в скрытую папку c указанием полного пути не осуществлен!\n')
        break
# 22. Переход в директорию, содержащую имя на кириллице с указанием полного пути:
while True:
    ...
    if m_r_pk == 0:
        with open(results, 'a') as m_r_pk_rep:
            m_r_pk_rep.write('22. Успех. Переход в директорию, содержащую имя на кириллице с указанием полного пути успешен\n')
        break
    else:
        with open(results, 'a') as m_r_pk_rep:
            m_r_pk_rep.write('22. Баг! Переход в директорию, содержащую имя на кириллице с указанием полного пути не осуществлен!\n')
        break
# 23. Переход в директорию, содержащую имя на латинице в разных регистрах с несоблюдением регистра:
while True:
    ...
    if m_r_preg == 0:
        with open(results, 'a') as m_r_preg_rep:
            m_r_preg_rep.write('23. Успех. Переход в директорию, содержащую имя на латинице в разных регистрах с несоблюдением регистра успешен\n')
        break
    else:
        with open(results, 'a') as m_r_preg_rep:
            m_r_preg_rep.write('23. Баг! Переход в директорию, содержащую имя на латинице в разных регистрах с несоблюдением регистра не осуществлен!\n')
        break
# 24. Переход в директорию, содержащую имя на латинице, кириллице, цифры, иные символы:
while True:
    ...
    if m_r_poth == 0:
        with open(results, 'a') as m_r_poth_rep:
            m_r_poth_rep.write('24. Успех. Переход в директорию, содержащую имя на латинице, кириллице, цифры, иные символы, успешен\n')
        break
    else:
        with open(results, 'a') as m_r_poth_rep:
            m_r_poth_rep.write('24. Баг! Переход в директорию, содержащую имя на латинице, кириллице, цифры, иные символы, не осуществлен!\n')
        break
